import{K as M,r as R,j as e,L as q,$ as z,S as H}from"./app-CTYZX1iD.js";import{B as f}from"./app-logo-icon-bWN_Ne22.js";import{A as Q}from"./app-layout-BqkgGdjN.js";import{I as J}from"./layout-DrlWGYfs.js";import"./index-CifmjVfu.js";import"./Combination-C27BhUmd.js";import"./index-BhvrERaG.js";import"./index-hleV0KyR.js";function Z(){const I=M(),{preview:N,company:s}=I.props,{customer:w,invoice_number:b,order:p,period_start:_,period_end:j,status:P,terbilang:g}=N,k=t=>t?new Date(t).toLocaleDateString("id-ID"):"-",l=t=>Number(t||0).toLocaleString("id-ID"),[A,T]=R.useState(()=>{var r;const t={};for(const a of p.order_items)for(const d of a.additional_products??[])t[`${a.id}:${d.id}`]=Number(((r=d.pivot)==null?void 0:r.quantity)??0);return t}),$=()=>{const t=document.getElementById("invoice-content");if(!t)return;const r=window.open("","_blank","width=800,height=600");if(!r){alert("Pop-up diblokir. Mohon izinkan pop-up untuk fitur print.");return}const a=Array.from(document.styleSheets).reduce((o,i)=>{try{const n=i.cssRules?Array.from(i.cssRules).map(c=>c.cssText).join(""):"";return o+`<style>${n}</style>`}catch{return o+`<link rel="stylesheet" href="${i.href}">`}},""),d=t.outerHTML;r.document.write(`
    <!DOCTYPE html>
    <html>
      <head>
  ...
  `),r.document.close(),r.onload=()=>{r.print()}},x=(t,r)=>A[`${t}:${r}`]??0,D=(t,r,a)=>{const d=Number.isFinite(a)&&a>=0?Math.floor(a):0;T(o=>({...o,[`${t}:${r}`]:d}))},y=Number(N.materai??0),m=(()=>{var d;let t=0;for(const o of p.order_items){t+=Number(o.price_value??0);for(const i of o.additional_products??[]){const n=Number(((d=i.pivot)==null?void 0:d.price_value)??i.price_value??0),c=x(o.id,i.id);t+=n*c}}const r=Math.round(t*.11),a=t+r+y;return{subtotal:t,ppn:r,grand_total:a}})(),[S,v]=R.useState(!1),C=p.order_items.flatMap(t=>(t.additional_products??[]).map(r=>({order_item_id:t.id,additional_product_id:r.id,quantity:x(t.id,r.id)}))),E={invoice_number:b,customer_id:w.id,order_id:p.id,period_start:_,period_end:j,subtotal:m.subtotal,ppn:m.ppn,materai:y,grand_total:m.grand_total,terbilang:g,order_item_ids:p.order_items.map(t=>t.id),additional_product_quantities:C},L=t=>{t.preventDefault(),v(!0),H.post("/invoices/store",E,{onSuccess:()=>v(!1),onError:()=>v(!1)})};return e.jsxs(Q,{children:[e.jsx(q,{title:"Preview Invoice"}),e.jsx("style",{children:`
                @media print {
