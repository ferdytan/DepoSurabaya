...
import { KeyRound, Palette, Sliders, User, Image as ImageIcon, Thermometer } from 'lucide-react';
import { type PropsWithChildren } from 'react';

export default function SettingsLayout({ children }: PropsWithChildren) {
    const { url, props } = usePage<SharedData>();
    const user = props.auth?.user as { role_id?: number; role_name?: string } | undefined;
    const isSuperAdmin = Boolean(
        user?.role_id === 1 ||
        (user?.role_name && /super/i.test(user.role_name)) ||
        ((user as any)?.role?.name && /super/i.test((user as any).role.name))
    );
    const isAdmin = isSuperAdmin || user?.role_id === 2 || (user?.role_name && /admin/i.test(user.role_name));
                            ...
                            return (
                                <Button
                                    ...
                                    className={cn(
                                        ...
                                        isActive
                                            ? 'bg-gray-100 text-gray-950 font-bold border border-gray-200 shadow-2xs hover:bg-gray-200/80 hover:text-black'
