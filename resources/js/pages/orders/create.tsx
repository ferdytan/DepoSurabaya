...
export default function CreateOrderWithMultiTemp({ customers, shippers, order_id }: PageProps) {
    ...
    const updateOrderItem = (index: number, field: keyof OrderItem, value: OrderItem[keyof OrderItem]) => {
        ...
        if (field === 'price_type') {
            ...
            newItems[index]['additional_product_prices'] = getAdditionalProductPrices(addIds, value as '20ft' | '40ft' | '45ft' | 'global' | undefined);
        }
    ...
    const hasDuplicateContainer = (index: number) => {
        ...
        return data.order_items.some((item, i) => i !== index && (item.container_number || '').trim().toUpperCase() === current);
    };
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        // Cek duplikasi kontainer sebelum post
        const containerNos = data.order_items.map((it) => (it.container_number || '').trim().toUpperCase()).filter(Boolean);
        const hasDup = containerNos.some((val, i) => containerNos.indexOf(val) !== i);
        if (hasDup) {
            setData('error', 'Terdapat nomor kontainer yang terduplikasi pada form ini. Mohon pastikan setiap nomor kontainer unik.');
            window.scrollTo({ top: 0, behavior: 'smooth' });
            return;
        }
    ...
    const getAdditionalProductPrices = (product_ids: string[], price_type?: '20ft' | '40ft' | '45ft' | 'global'): string[] => {
        return product_ids.map((id) => {
            const product = customerProducts.find((p) => p.id.toString() === id);
            let price = '0';
            if (product) {
                ...
                else if (price_type === '45ft') price = product.custom_price_45ft ?? '0';
                                    ...
                                    {useOrderId ? (
                                        <div className="space-y-1.5 max-w-md">
                                            ...
                                            <p className="text-[11px] text-gray-400">
                                                Nomor order ini dibuat otomatis oleh sistem saat dokumen disimpan.
                                            </p>
                                        </div>
                                    ) : (
                                        <div className="space-y-1.5 max-w-md">
                                                                        ...
                                                                        >
                                                                            <input
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    ...
                                                                                    let updated = item.additional_product_ids?.slice() || [];
                                                                                    if (checked) {
                                                                                        if (!updated.includes(pId)) updated.push(pId);
                                                                                    } else {
                                                                                        updated = updated.filter((id) => id !== pId);
                                                                                    }
                                                                                    updateOrderItem(idx, 'additional_product_ids', updated);
                                                                                    const prices = getAdditionalProductPrices(
                                                                                        updated,
                                                                                        item.price_type as '20ft' | '40ft' | '45ft' | 'global' | undefined,
                                                                                    );
                                                                                    updateOrderItem(idx, 'additional_product_prices', prices);
                                                                                }}
